...
export default class NavigationGraph {
    ...
    buildGraph(plots, crosswalkInfos) {
        ...
        // Garder ceil pour s'assurer que la grille est assez grande même avec round
    ...
    gridToWorld(gridX, gridY) {
        // Le +0.5 n'est plus strictement nécessaire si worldToGrid utilise round,
        // car gridX/gridScale devrait déjà être proche du centre.
        // Mais le garder ne fait pas de mal et assure le centre de la cellule.
        const worldX = (gridX + 0.5 - this.offsetX) / this.gridScale;
        const worldZ = (gridY + 0.5 - this.offsetZ) / this.gridScale;
        return new THREE.Vector3(worldX, this.sidewalkHeight + 0.05, worldZ); // Légèrement au-dessus
    }
	...
	markSidewalksArea(plots, sidewalkW) {
        ...
        plots.forEach(plot => {
            ...
            // Itérer sur les cellules de grille. Utiliser < max car l'indice max est inclusif avec round? Non, une boucle standard suffit.
            for (let gx = outerMinGridX; gx < outerMaxGridX; gx++) {
                for (let gy = outerMinGridY; gy < outerMaxGridY; gy++) {
                    const isOutsideInner = (gx < innerMinGridX || gx >= innerMaxGridX || gy < innerMinGridY || gy >= innerMaxGridY);
                    if (isOutsideInner) {
	...
	drawLineOnGrid(start, end, thickness) {
        ...
        while (true) {
             if (dx > -dy) {
                 for (let i = -halfThicknessFloor; i <= halfThicknessCeil; i++) { if (this.markCell(x0, y0 + i)) markedCount++; }
             } else {
                 for (let i = -halfThicknessFloor; i <= halfThicknessCeil; i++) { if (this.markCell(x0 + i, y0)) markedCount++; }
            }
	...
	markCell(x, y) {
        if (this.isValidGridCoord(x, y)) {
             if (!this.grid.isWalkableAt(x,y)){
                 this.grid.setWalkableAt(x, y, true); return true;
             }
        } return false;
    }
    ...
    getClosestWalkableNode(worldPos) {
        ...
        const startGrid = this.worldToGrid(worldPos.x, worldPos.z); // Utilise round maintenant
    ...
    createDebugVisualization(targetGroup) {
        ...
        const visualCellSize = cellSizeInWorld * 0.85;
        const planeGeom = new THREE.PlaneGeometry(visualCellSize, visualCellSize);
        const geometries = [];
        for (let y = 0; y < this.gridHeight; y++) {
            for (let x = 0; x < this.gridWidth; x++) {
                if (this.grid.isWalkableAt(x, y)) {
                    const worldPos = this.gridToWorld(x, y); // Utilise toujours +0.5 pour le centre visuel
                    const matrix = new THREE.Matrix4();
                    matrix.makeRotationX(-Math.PI / 2);
                    matrix.setPosition(worldPos.x, worldPos.y - 0.03, worldPos.z);
